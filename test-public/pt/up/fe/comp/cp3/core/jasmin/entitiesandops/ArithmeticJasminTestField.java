...
public class ArithmeticJasminTestField extends BaseJasminTestEnv {
    private static final String BASE_PATH = "pt/up/fe/comp/cp3/core/jasmin/entitiesandops/arithmetic/";

    public ArithmeticJasminTestField(BaseJasminTestEnv.InputSource inputSource) {
        super(inputSource, BASE_PATH);
    }

    @Test
    public void AddLiteralWithField() {
        var res = toJasmin("AddLiteralWithField");
        var fieldValue = 3;
        var expected = 4;
        var instance = res.newInstance();
        res.setFieldValue(instance, "b", fieldValue);
